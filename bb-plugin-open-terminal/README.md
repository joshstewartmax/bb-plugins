# bb-plugin-open-terminal

Adds an "Open terminal" button to the thread composer. Clicking it creates a
terminal session scoped to the open thread, so it starts in the project and
...
```
bb plugin reload open-terminal
```

## Configure

```
bb plugin config open-terminal
bb plugin config open-terminal set greeting hi
```
